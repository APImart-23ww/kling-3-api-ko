# Kling v3 API 한국어 가이드: 모델 ID, 초당 가격, 호출 예제
...
## APIMart로 Kling v3를 호출하는 이유

- **키 하나로 전체 카탈로그.** 같은 base URL과 인증 헤더로 Kling v3를 포함한 300개 이상의 이미지·영상·언어 모델에 접근하며, 전환은 `model` 필드만 바꾸면 됩니다.
...
curl --request POST \
  ...
  --data '{"model":"kling-v3", "prompt":"a cozy reading nook by a rainy window, warm lamp light", "n":1}'
```
